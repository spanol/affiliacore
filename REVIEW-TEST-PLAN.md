...
**Fase 2 COMPLETA** (commits `bca03af`, `51a002d`; 240 testes verdes + 42 de rules no `test:rules`):
- **P0.8 — regressão de `firestore.rules` no emulator** (`bca03af`): `test/rules/firestore-rules.spec.ts` (42 asserts) testa o arquivo REAL afirmando o comportamento SEGURO (não o POC vulnerável-vs-patched, já obsoleto). Cobre users create/update (privesc de role; `affiliateId`/`isSpecial` server-only travados = R6; fluxo legítimo de editar nome intacto), `affiliate_configs` admin-only (R5), 9 coleções admin-only (PII de pagamento, logs, casas, house_results, links, partner keys), 5 signed-in-read (escrita só admin), `direct_messages` escopado ao `recipientUid`, `contacts` create público restrito ao shape. Infra: bloco `emulators` no `firebase.json`, `vitest.rules.config.ts` (env node, `include test/rules/**` — fora do glob `src/**` ⇒ `npm test` não roda sem emulator), script `test:rules` (`firebase emulators:exec`). Requer o firebase CLI + Java (convenção do repo p/ ops de firebase). **Não** adiciona `firebase-tools` ao lockfile (consistente com o CLI global do repo; o JAR é baixado/cacheado fora do npm de qualquer forma).
- **P1.4 — `createApp(deps)` + supertest** (`51a002d`): `server.ts` extrai `export function createApp(deps)` de `startServer()` (monta middlewares+rotas sem ouvir porta nem montar Vite/estático; `startServer` só inicializa o Admin via `initAdmin()`, chama `createApp` e adiciona a camada de ambiente). `ServerDeps` injeta `adminApp`/`adminDb`/`fetchImpl`; init do Admin sai do escopo de módulo; 4 `fetch` do proxy/sync via `fetchImpl`; guard de auto-start (`VITEST`/`NODE_ENV`). `server.test.ts` (20 testes, env node): R4 (IDOR do proxy — endpoint≠results→403; results escopa ao próprio id; id alheio→403 interseção vazia; admin irrestrito), R13 (requireAdmin 401/403/200), R16 (house-results — agregado `affiliateId null` não vaza ao afiliado), R20 (convite 404/410 used/410 expired/201 + accept cria role client e marca single-use + rate limit 31ª→429), R25 (create-user valida enum de role — **bug fechado**: gravava `String(role)` verbatim). Refactor preserva o comportamento de produção (boot smoke OK; `server.ts` **roda código antigo até reiniciar o processo**).
